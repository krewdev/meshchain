...
2. **Public finality is TCP PoA** — Meshtastic air path carries **submit + tip + BalQuery + compact AirBlockAck** (type 14). Cloud seed still finalizes on TCP; LoRa-only validator set is Phase 2–3 ([AIR_FINALITY.md](AIR_FINALITY.md)).  
...
**Meshtastic:** [docs/MESHTASTIC.md](MESHTASTIC.md) · [AIR_FINALITY.md](AIR_FINALITY.md) · `mesh balance --air` · `mesh send … --air` · `tools/mesh_radio_relay.py` · `scripts/e2e_air_path.sh`
